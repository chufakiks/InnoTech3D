...
def pre_process(project):
    point_cloud_file = 'data/TrainingSet/{}/{}.xyz'.format(project, project)
    bounding_box_file = 'data/TrainingSet/{}/{}.csv'.format(project, project)
...
def extract_point_features(box, points, project, metadata_list):
    ...
    label = str(box['Label']) 
    id = str(box['ID'] )
    file_path = os.path.join(directory, "{} {} {}.xyz".format(project, label, id))
    xyzfile = open(file_path, "w")

    # Write the selected points to the file
    selected_points.to_csv(xyzfile, sep=' ', index=False, header=False)
    xyzfile.close()
    print("{}, {}, points inside: {}".format(project, label, len(selected_points)))
    ...
    metadata_list.append({
        ...
        "pc_filename": f"{project} {label} {id}.xyz"
    })
